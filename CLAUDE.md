...
> 이 파일은 1~5단계 산출물(문제정의/시장조사/기능명세/시스템설계/ADR/스캐폴드)을 구현 단계에서 즉시 참조 가능하도록 압축한 것이다. 전체 근거는 `docs/`와 프로젝트 상위 문서(problem_definition.md 등)를 참조하되, **여기 없는 내용을 상위 문서에서 찾아야 한다면 이 파일이 불완전한 것 — 발견 시 이 파일에 추가할 것.**
