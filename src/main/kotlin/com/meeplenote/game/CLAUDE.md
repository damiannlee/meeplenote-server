...
- **M8 보류**: BGG XML API2가 Cloudflare 봇 차단으로 401 반환, 무인증 온디맨드 조회 불가. 상세·재개 시 확장할 캐시 필드 범위는 **ADR-003을 단일 출처로 참조** (재서술 안 함). 코드는 `feat/m8-bgg-cache` 브랜치 보존 — `main`(M2가 이후 별도 재구현되며 갈라짐) 대비 크게 뒤처져 재개 시 리베이스 필요.
